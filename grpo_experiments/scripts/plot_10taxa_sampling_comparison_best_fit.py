#!/usr/bin/env python3
"""Compare four 10-taxa methods with per-method linear least-squares fits."""
...
COMPARISON_DIR = REPO_ROOT / "grpo_experiments/comparisons/10taxa"
BLUE = "#1976d2"
GRAY = "#555555"
...
PLAIN_GRPO_RUN = (
    ...
    "20260804_133452_plain_grpo_10taxa_full_sig_b4096_shift5000_grpo"
)
COUNT_IPS_RUN = (
    REPO_ROOT
    / "grpo_experiments/learned_reverse_runs/"
    "20260804_011029_count_ips_10taxa_full_sig_b4096_shift5000_ips_grpo"
)
LEARNED_REVERSE_RUN = (
    REPO_ROOT
    / "grpo_experiments/learned_reverse_runs/"
    "20260803_124837_learned_reverse_10taxa_mlp_shifted_linear_b4096_"
    "learned_reverse_ips_grpo"
)
GFLOWNET_RUN = (
    REPO_ROOT
    / "og_code/experiments/full_model/"
    "20260804_112555_20260803_124841_phylgfn_logreward_10taxa_g4096_"
    "1m_full_replay_op3277_r819_rb4096_resumed"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Maximum displayed points per panel; 0 displays every sample.",
    )
...
def methods() -> list[Method]:
    return [
        Method(
            title="Plain GRPO",
            samples=PLAIN_GRPO_RUN / "sampled_full_diagnostics_1000000.npz",
            estimator_label=r"Empirical $\hat q(x)=\mathrm{count}(x)/N$",
            kind="empirical_signature",
        ),
        Method(
            title="Original IPS",
            samples=COUNT_IPS_RUN / "sampled_full_diagnostics_1000000.npz",
            estimator_label=r"Empirical $\hat q(x)=\mathrm{count}(x)/N$",
            kind="empirical_signature",
        ),
        Method(
            title="GFlowNet (trajectory balance)",
            samples=GFLOWNET_RUN
            / "plots/reward_probability_eval_1000000/"
            / "og_gflownet_reward_probability_samples.npz",
            estimator_label=r"$P_F(\tau)/P_B(\tau)$",
            kind="gflownet",
        ),
        Method(
            title="Learned reverse IPS (new proposed)",
            samples=LEARNED_REVERSE_RUN / "sampled_full_diagnostics_1000000.npz",
...
def scatter_style(n_points: int) -> tuple[float, float]:
    if n_points <= 2:
        return 40.0, 0.9
    if n_points < 5_000:
        return 12.0, 0.35
...
def _signature_pairs(
    topology_index: np.ndarray,
    reward_for_hash: np.ndarray,
) -> np.ndarray:
    signatures = np.empty(
        len(topology_index),
        dtype=[("topology", np.int64), ("reward_milli", np.int64)],
    )
    signatures["topology"] = topology_index.astype(np.int64)
    signatures["reward_milli"] = np.rint(reward_for_hash * 1000.0).astype(np.int64)
    return signatures
...
def load_empirical_signature_frequency(
    ...
    signatures = _signature_pairs(topology_index, reward)
...
def load_method(method: Method) -> tuple[np.ndarray, np.ndarray, int, int]:
    ...
    with np.load(method.samples) as payload:
        ...
        if method.kind == "gflownet":
            reward = np.exp(payload["log_reward"].astype(np.float64))
            log_probability = payload["log_probability"].astype(np.float64)
            # Match 5-taxa hashing: signature from topology + raw log-likelihood.
            hash_reward = payload["raw_log_likelihood"].astype(np.float64)
        else:
            reward = payload["log_score"].astype(np.float64)
            log_probability = (
                payload["log_pf"].astype(np.float64)
                - payload["log_q_reverse"].astype(np.float64)
            )
            hash_reward = reward
        unique_signatures = int(len(np.unique(_signature_pairs(topology_index, hash_reward))))
        return reward, np.exp(log_probability), unique_signatures, len(reward)
...
def plot_panel(
    ...
    *,
    ...
    selected = select_points(len(reward), scatter_points, seed)
...
def main() -> None:
    ...
    for index, (ax, method, (reward, probability, unique_signatures, sample_count)) in enumerate(
        zip(axes.ravel(), method_specs, loaded, strict=True)
    ):
    ...
    fig.supxlabel(r"Terminal reward: $R(x)=5000+\log L(x)$", fontsize=14)
    fig.supylabel("Outcome probability estimate", fontsize=14)
    fig.suptitle(
        "10-taxa outcome probability versus reward",
        fontsize=16,
    )
