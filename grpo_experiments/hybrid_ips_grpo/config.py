"""
Configuration for hybrid IPS-GRPO:
  fresh pi_old samples + best-tree replay samples, then multi-cycle IPS + policy IS updates.
"""
...
@dataclass
class HybridIPSExperimentConfig:
    """Knobs for hybrid best-tree replay + IPS + policy-IS GRPO."""
    ...
    # IPS (Sinha et al. 2026).
    ips_prob_floor: float = 1e-6
    ips_start_round: int = 0
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Hybrid IPS-GRPO: sample fresh trees under pi_old + replay best trees, "
            "then run multiple IPS + IS-weighted update cycles."
        )
    )
    p.add_argument("--cfg", dest="cfg_path", default=HybridIPSExperimentConfig.cfg_path)
    p.add_argument("--dataset", dest="dataset_path", default=HybridIPSExperimentConfig.dataset_path)
    p.add_argument("--output", dest="output_root", default=HybridIPSExperimentConfig.output_root)
    ...
    g = p.add_argument_group("optimizer / IPS / IS")
...
def config_from_args(args: argparse.Namespace) -> HybridIPSExperimentConfig:
    return HybridIPSExperimentConfig(
        ...
        ips_start_round=args.ips_start_round,
