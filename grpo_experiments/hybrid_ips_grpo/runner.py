"""Runner for hybrid IPS-GRPO: fresh + best-tree replay with IPS + policy IS cycles."""
...
from grpo_experiments.hybrid_ips_grpo.config import HybridIPSExperimentConfig
...
def _build_ips_trainer(exp_cfg: HybridIPSExperimentConfig, generator, cfg) -> IPSGRPOTrainer:
    ...
    print(
        f"Hybrid-IPS-GRPO optimizer: {len(params)} param groups, {n_params:,} parameters  "
...
def _init_run_state(exp_cfg: HybridIPSExperimentConfig, output_dir: str, cfg):
...
def _run_hybrid_ips_policy_is(
    exp_cfg: HybridIPSExperimentConfig,
    device: str,
    output_dir: str,
    cfg,
    all_seqs,
    ...
    trainer = _build_ips_trainer(exp_cfg, generator, cfg)
    ...
    for resample_round in range(start_round, exp_cfg.effective_resample_rounds):
        ...
        if exp_cfg.ips_start_round > 0:
            ips_metrics["ips_start_round"] = float(exp_cfg.ips_start_round)
        traj_logger.log_batch(
            ...
            extra={
                ...
                "ips_prob_mean": ips_metrics.get("ips_prob_mean"),
            },
        )
...
def run_experiment(exp_cfg: HybridIPSExperimentConfig) -> str:
    ...
    if exp_cfg.ips_start_round > 0:
        print(f"  ips_start_round={exp_cfg.ips_start_round}")
    ...
    return _run_hybrid_ips_policy_is(exp_cfg, device, output_dir, cfg, all_seqs)
