...
SAMPLING_PLOT_NAME = "sampling_scatter.png"
LOG_SCATTER_NAME = "log_scatter.png"
...
def run_dir_name(*, color_profile: str, group_size: int, run_name: str | None = None) -> str:
    base = f"{run_stamp()}_{color_profile}_gs{group_size}"
    if run_name:
...
def new_run_dir(
    *,
    ...
    run_dir = root / run_dir_name(color_profile=color_profile, group_size=group_size, run_name=run_name)
...
class RunPaths:
    ...
    @property
    def log_scatter(self) -> Path:
        return self.run_dir / LOG_SCATTER_NAME
...
def config_payload(
    train_config,
    *,
    color_profile_spec: dict[str, object],
    extra: dict[str, Any] | None = None,
) -> dict[str, Any]:
    payload: dict[str, Any] = {
        "created_at": datetime.now(timezone.utc).isoformat(),
        "train_config": asdict(train_config),
        "color_profile_spec": color_profile_spec,
    }
    if extra:
