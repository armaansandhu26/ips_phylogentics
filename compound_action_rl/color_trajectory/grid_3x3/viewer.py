"""Matplotlib viewer for the 3x3 color-trajectory GridEnv."""
...
class ColorGridViewer:
    def __init__(
        ...
        *,
        ...
        self.fig, axes = plt.subplots(1, 2, figsize=(9, 4.5))
        self.ax_grid, self.ax_opt = axes
        self.fig.canvas.manager.set_window_title(title)
        self.fig.patch.set_facecolor("#f5f6fa")
        self.fig.suptitle(
            f"3x3 debug — max {self.env.max_episode_steps} steps",
            fontsize=13,
            fontweight="bold",
            y=0.98,
        )
    ...
    def step_once(self) -> None:
        ...
        if done:
            ...
            if self.env._at_goal():
                ...
                end_reason = f"Timed out at {tuple(state)} (max {self.env.max_episode_steps} steps)"
            self.status_text = (
            ...
            self.status_text = (
                ...
                f"net1={self.last_move}, net2={self.last_color}, agent at {tuple(state)}."
            )
        self.draw()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="View 3x3 GridEnv rollouts.")
    ...
    if args.checkpoint:
        ...
        title = "3x3 Color Trajectory — Learned GRPO Policy"
    else:
        agent = make_random_agent(seed=args.seed)
        title = "3x3 Color Trajectory — Random Agent"
