"""Train IPS-GRPO on the 3x3 color grid."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train IPS-GRPO on the 3x3 color grid.")
    ...
    parser.add_argument(
        "--save-path",
        type=str,
        default="checkpoints/ips_grpo.pt",
        help="Where to save the trained IPS-GRPO checkpoint.",
    )
    ...
    print(
        f"3x3 IPS-GRPO: max_episode_steps={env.max_episode_steps}, "
        f"advantage_mode={args.advantage_mode}, p_hat_mode=group, "
        f"group_size={args.group_size}, num_groups={args.num_groups}, "
        f"clip_ratio={args.clip_ratio}, p_eps={args.p_eps}, device={device}"
    )
