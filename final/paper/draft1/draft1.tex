...
of $0.262$ and $0.270$, and cover $4056$ and $4043$ terminals. These two are
...
\paragraph{Regime where counting works ($H=8$, $G=256$).}
To isolate the effect of group size, we also run the same reward landscape on
an $8\times8$ grid ($64$ terminals) with batch size $G=256$ and 2{,}000
epochs, averaged over three seeds (Table~\ref{tab:grid-b256}). Here
$G > |\mathcal{X}|$, so repeated outcomes appear in every group and the count
estimate $\hat{p}(x)$ is informative. Count IPS-GRPO recovers all four modes
($\ell_1 = 0.226 \pm 0.007$), confirming that IPS works when counting is
possible. MIPS-GRPO also recovers all four modes but with substantially lower
$\ell_1$ ($0.057 \pm 0.007$), showing that the learned reverse weight still
helps even when counts are available. GRPO continues to collapse to one mode.
Figure~\ref{fig:grid-b256} shows seed~0 terminal distributions and training
curves; per-seed numbers are in Table~\ref{tab:grid-b256-seeds}.

\input{tables/tab_hypergrid64}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{hypergrid/terminal_distribution_comparison.png}
...
\begin{subfigure}{0.49\textwidth}
  \includegraphics[width=\textwidth]{hypergrid/modes_found_vs_samples_logx.png}
  \caption{log scale}
\end{subfigure}
\hfill
\begin{subfigure}{0.49\textwidth}
  \includegraphics[width=\textwidth]{hypergrid/modes_found_vs_samples.png}
...
\centering
\begin{subfigure}{0.49\textwidth}
  \includegraphics[width=\textwidth]{hypergrid/training_l1_curve.png}
  \caption{evaluation $\ell_1$}
\end{subfigure}
\hfill
\begin{subfigure}{0.49\textwidth}
  \includegraphics[width=\textwidth]{hypergrid/training_reward_curve.png}
...
\input{tables/tab_hypergrid8_b256}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{hypergrid_b256/terminal_distribution_comparison.png}
\caption{Terminal distributions on the $8\times8$ hyper-grid with $G=256$
(seed~0). Count IPS-GRPO and MIPS-GRPO both cover all four modes; GRPO
collapses. MIPS-GRPO matches the target most closely ($\ell_1 = 0.049$).}
\label{fig:grid-b256-dist}
\end{figure}

\begin{figure}[htbp]
\centering
\begin{subfigure}{0.49\textwidth}
  \includegraphics[width=\textwidth]{hypergrid_b256/modes_found_vs_samples_logx.png}
  \caption{modes found (log scale)}
\end{subfigure}
\hfill
\begin{subfigure}{0.49\textwidth}
  \includegraphics[width=\textwidth]{hypergrid_b256/training_l1_curve.png}
  \caption{evaluation $\ell_1$}
\end{subfigure}
\caption{Hyper-grid $H=8$, $G=256$ (seed~0). Left: mode discovery against
samples drawn. Count IPS-GRPO and MIPS-GRPO reach four modes quickly; GRPO
stays at one. Right: $\ell_1$ to target during training.}
\label{fig:grid-b256}
\end{figure}
...
\input{tables/tab_phylo_setup}

\input{tables/tab_phylo_matched}

\input{tables/tab_phylo_extended}

At 5 taxa both multiplicity-aware methods recover the ideal distribution and
cover all 105 topologies (Table~\ref{tab:phylo-extended}), while count IPS-GRPO
finds 3 topologies and GRPO collapses to 1. At 10 taxa the learned reverse
policy is ahead on correlation at equal coverage (Table~\ref{tab:phylo-matched}),
...
near-perfect correlation on both linear and log-log scales ($r = 0.977$) against
no meaningful probability--reward relationship for GFlowNet ($r = 0.002$ linear,
$r = 0.024$ log-log). We report \emph{matched transforms} for both methods so
that linear and log-log panels are directly comparable
(Table~\ref{tab:phylo-matched}; Figure~\ref{fig:phylo-27}).

Figures~\ref{fig:phylo-5} and~\ref{fig:phylo-10} show the probability--reward
relationship for all four methods at 5 taxa and the matched-transform comparison
at 10 taxa. At 5 taxa, both multiplicity-aware methods track the target on
log-log axes, whereas GRPO and count IPS-GRPO cover only a small fraction of the
outcome space. At 10 taxa, MIPS-GRPO remains tightly aligned with the ideal
linear relationship while GFlowNet TB shows substantially more dispersion.

\input{tables/tab_uniform_ablation}

At 10 taxa, a uniform-$P_B$ ablation (Table~\ref{tab:uniform-ablation}) shows
that IPS weights remain tractable (ESS $\approx 0.995$) without fitting
$q_\phi$, but linear-scale calibration requires the learned reverse policy.

\begin{figure}[htbp]
\centering
\begin{subfigure}[t]{0.49\textwidth}
  \includegraphics[width=\textwidth]{phylo/5taxa_grpo_p_vs_r.png}
  \caption{GRPO (collapsed)}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.49\textwidth}
  \includegraphics[width=\textwidth]{phylo/5taxa_count_ips_p_vs_r.png}
  \caption{Count IPS-GRPO}
\end{subfigure}
\\[0.5em]
\begin{subfigure}[t]{0.49\textwidth}
  \includegraphics[width=\textwidth]{phylo/5taxa_mips_grpo_log_p_vs_log_r.png}
  \caption{MIPS-GRPO (log-log)}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.49\textwidth}
  \includegraphics[width=\textwidth]{phylo/5taxa_gflownet_log_p_vs_log_r.png}
  \caption{GFlowNet TB (log-log)}
\end{subfigure}
\caption{Five-taxa outcome probability versus terminal reward from 1M samples
(\texttt{final/runs/5taxa\_noreplay}). GRPO collapses and count IPS-GRPO covers
few signatures; GFlowNet TB and MIPS-GRPO both recover the
reward-proportional relationship on log-log axes.}
\label{fig:phylo-5}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.96\textwidth]{phylo/figure6_10taxa_matched_transform.png}
\caption{Ten-taxa matched-transform comparison (1M samples). Linear and log-log
Pearson $r$ values match Table~\ref{tab:phylo-matched}. MIPS-GRPO closely
follows the ideal relationship on both scales; GFlowNet TB has greater
dispersion on linear axes.}
\label{fig:phylo-10}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{phylo/figure6_27taxa_matched_transform.png}
\caption{Twenty-seven taxa on full DS1, evaluated with 1M samples under matched
transforms. MIPS-GRPO achieves Pearson $r=0.977$ on both linear and log-log
axes; GFlowNet TB has no meaningful probability--reward relationship
($r=0.002$ linear, $r=0.024$ log-log).}
\label{fig:phylo-27}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{phylo/training_all_taxa.png}
...
\input{tables/tab_reverse_training}
