% Matched-transform headline comparison (MIPS-GRPO vs GFlowNet).
% Source: final/paper/table2.json (manifest-backed runs).
\begin{table}[htbp]
\centering
\caption{Matched-transform Pearson correlations between pathwise terminal
probability and reward (1M self-sampled trajectories). Both methods report
linear $P(x)$ vs.\ $R(x)$ and log-log $\log P(x)$ vs.\ $\log R(x)$ under the
same transform within each column.}
\label{tab:phylo-matched}
