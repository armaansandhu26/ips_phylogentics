...
def train(device_rank, world_size, ddp, cfg, paths_data, verbose=False):
    ...
    if resume_path:
        ...
        if cfg_eval.FIXED_STATES:
            ...
            if eval_folders:
                ...
                print(
                    "no saved evaluation states found; evaluator will initialize "
                    "a new fixed-state set"
                )
    ...
    log_progress_freq = training_cfg.LOG_PROGRESS_FREQ
    for epoch_id in range(epoch_to_start, epochs_num):
        ...
        train_ret = train_epoch(cfg, epoch_id, generator, data_loader, logger, log_results, verbose)
        epoch_num = epoch_id + 1
        is_last_epoch = epoch_num == epochs_num
        should_checkpoint = (epoch_num % checkpoint_freq == 0) or is_last_epoch
        should_log_progress = (epoch_num % log_progress_freq == 0) or is_last_epoch

        # save model and logging data
        if device_rank == 0:
            if should_log_progress:
                loss_val = train_ret['loss'] if train_ret is not None else float('nan')
                log_z = generator.compute_log_Z().item()
                print(
                    f'Epoch {epoch_num}/{epochs_num} | loss: {loss_val:.4f} | log_Z: {log_z:.4f}',
                    flush=True,
                )

            if should_checkpoint:
                save_path = os.path.join(
                    paths_data['output_path'], 'checkpoints', "checkpoint_%06d.pt" % (epoch_id,)
                )
            ...
            # run evaluation
            if should_checkpoint and epoch_id % evaluation_freq == 0:
